...
const RaffleAdminPage: FC<RaffleAdminPageProps> = () => {
  ...
  const onChangeData = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name.includes("entry")) {
      const temp = { ...data };
      temp.entry[parseInt(name.match(/\[(\d+)\]/)?.[1] ?? "0")] = parseInt(value);
      setData(temp);
    }

    if (name.includes("points")) {
      const temp = { ...data };
      temp.points[parseInt(name.match(/\[(\d+)\]/)?.[1] ?? "0")] = parseInt(value);
      setData(temp);
    }
  };
  ...
  const onEdit = (id: string) => {
    ...
    setData({
      ...
      entry: raffle.conditions.map((c: any) => c.entry),
      points: raffle.conditions.map((c: any) => c.points),
